Create a single horizontal sprite strip for the Codex app digital pet `mimi` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Mimi is a complete-body cream-golden anime kitten, tiny and cute, with round face, small triangular ears, short paws, curled fluffy tail, dot eyes, tiny pink nose, gentle expression, and no engineering symbols or tools..
